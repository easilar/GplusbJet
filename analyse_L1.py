...
HLT_36_json = afs_dir+"json/prescale_jsons/HLT_Photon36_R9Id90_HE10_IsoM_prescale1_2016_36p12.json"
if year == 2016:
   act_Lumi = 36.47
...
orig_dir = sdict["dir"]+"/"
targetdir = targetdir_mainpath+"/data/"+str(year)+"/"+sname+"/"+targetdir_suffix+"/"+sdict["dir"].split("/")[-3]+"/"
if not os.path.exists(targetdir):
	os.makedirs(targetdir)
data = json.load(open(cert_json))
data_HLT = json.load(open(HLT_36_json))
...
pfile2 = afs_dir+"/json/prescale_jsons/pickles/HLT_Photon36_prescale1.pkl"
		...
		for lumiBlock in data_HLT[str(int(run))]:
			if (lumi >= lumiBlock[0] and lumi <= lumiBlock[1]) : 
				if not lumi in HLT_prescale1_dict[str(int(run))] : 
					...
					if weight_HLT_36 < 1.1 : 
						print("THIS EVENT WILL BE SKIPPED : ", run,lumi,event,weight_HLT_36)
						continue
					else : 
						print("I WILL KEEP THIS EVENT 1st: ", run,lumi,event,weight_HLT_36)
						tree.Fill()
				else : 
						
					print("I WILL KEEP THIS EVENT 2nd: ", run,lumi,event,weight_HLT_36)
					tree.Fill() 	
'''				
	###L1 Starts###
