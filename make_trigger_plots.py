...
#start data
h_data_den = getPlotFromChain(c, plot['var'], (50,0,200), cutString =den_cut , weight = "(1)" ,addOverFlowBin='both',variableBinning=plot["bin_set"]) 
h_data_num = getPlotFromChain(c, plot['var'], (50,0,200), cutString =num_cut , weight = "(1)" ,addOverFlowBin='both',variableBinning=plot["bin_set"]) 
...
platho = round(fitResult.GetParameter(0),4)
plato_X = 0
for i in range(50):
	if round(fitResult(threshold+i*0.5),4) == platho :
		print(threshold+i*0.5,fitResult(threshold+i*0.5))
...
tex.DrawLatex(0.2,0.75,"Plateau: "+str(round(platho,4))+", Reached at "+str(round(plato_X,2))+" GeV")
cb.cd()
cb.Draw()
